...
if not active_places and not use_sample:
  ...
  if use_sample:
    ...
    # --- 会場選択後に開催中レースを取得するメッセージ表示 ---
    with st.spinner(f"⏳ {selected_place}の開催中・未締切レースを取得中..."):
    ...
    with col_race:
      if not purchasable_races:
        ...
        selected_race_str = st.selectbox(
            "対象レース（締切前のみ）", race_options
        )
  ...
  if submit_btn:
    if use_sample:
      ...
      with st.spinner(
          f"📊 {display_place_name} {display_race_no}R"
          " のリアルタイムオッズ・レースデータを解析中..."
      ):
        try:
        ...
        if df_raw is None or df_raw.empty:
          ...
          for combo, ai_prob in trifecta_probs.items():
            ...
            predictions.append({
                "買い目": combo,
                "オッズ_num": odds,
                "オッズ": f"{odds:.1f}倍" if odds != 10.0 else "取得不可",
    ...
    if df_all is not None:
      st.toast("✅ 最新オッズとAI予想結果を出力しました！", icon="🎉")
